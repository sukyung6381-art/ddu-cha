...
import { monthLabel, type Rankings } from "@/lib/rankings";
...
export function HallOfFame({ rankings: r }: { rankings: Rankings }) {
    ...
    <div className="mx-auto w-full max-w-6xl px-6 py-16 sm:py-24">
      ...
      <section className="mb-16">
        <div className="mb-6 flex items-baseline justify-between">
          ...
          <span className="hidden text-xs text-zinc-500 sm:inline">매 연말 종가 기준 10년 연속 상승</span>
        </div>
          ...
          <table className="w-full">
            <thead className="border-b border-zinc-200 bg-zinc-50 text-xs text-zinc-500 dark:border-zinc-800 dark:bg-zinc-950">
              <tr>
                ...
                <th className="hidden px-6 py-3 text-right md:table-cell">10년 전</th>
                <th className="hidden px-6 py-3 text-right md:table-cell">현재</th>
                <th className="px-6 py-3 text-right">배수</th>
                <th className="hidden px-6 py-3 text-right sm:table-cell">수익률</th>
              </tr>
            </thead>
            <tbody>
              {r.moonshotTop10.map((e, i) => {
                ...
                return (
                  ...
                  >
                    ...
                    <td className="hidden px-6 py-4 text-right tabular-nums text-zinc-500 md:table-cell">
                      {e.market === "KR" ? "₩" : "$"}
                      {e.from.close.toLocaleString()}
                    </td>
                    <td className="hidden px-6 py-4 text-right tabular-nums md:table-cell">
                      {e.market === "KR" ? "₩" : "$"}
                    ...
                    <td className="hidden px-6 py-4 text-right tabular-nums text-emerald-600 dark:text-emerald-400 sm:table-cell">
                      {formatPct(e.totalReturnPct)}
                    </td>
                  </tr>
                );
      ...
      <section className="mb-16">
        <div className="mb-6 flex items-baseline justify-between">
          ...
          <span className="hidden text-xs text-zinc-500 sm:inline">최대 12년치 표본</span>
        </div>
        ...
        {r.monthWorst.length > 0 && (
          <div className="mt-8 rounded-xl border border-red-200 bg-red-50/50 p-5 dark:border-red-950 dark:bg-red-950/20">
            <h4 className="mb-3 text-sm font-medium text-red-700 dark:text-red-300">
